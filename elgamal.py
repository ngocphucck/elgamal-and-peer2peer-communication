...
if __name__ == '__main__':
    ...
    keys = generate_keys(iNumBits=32, iConfidence=52)	
    priv = keys['privateKey']
    print('Private key: ', priv.p, " ", priv.g, " ", priv.x)
    pub = keys['publicKey']
    print('Public key: ', pub.p, " ", pub.g, " ", pub.h)
    message = "My name is Ryan.  Here is some french text:  Matre Corbeau, sur un arbre perché.  Now some Chinese: 鋈 晛桼桾 枲柊氠 藶藽 歾炂盵 犈犆犅 壾, 軹軦軵 寁崏庲 摮 蟼襛 蝩覤 蜭蜸覟 駽髾髽 忷扴汥" 
    cipher = encrypt(pub, message)
    plain = decrypt(priv, cipher)

    print("Plain text: ", message)
    print("Cipher: ", cipher)
    print("Plain receive text: ", plain)
    print(message == plain)
